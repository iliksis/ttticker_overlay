...
import { setOverlayState, type OverlayState } from '$lib/server/state.svelte.js';

const eventBus = new ServerEventBus();

export const POST = async ({ request }) => {
	try {
		const data: { table: number; state: OverlayState } = await request.json();
		setOverlayState(data.table, data.state);
		eventBus.emit('overlay', data);
		return json(
			{ success: true },
			{
				status: 200
			...
			handleNewData = (event: any) => {
				...
				if (!isControllerClosed) {
					try {
						...
						eventBus.removeEventListener('overlay', handleNewData);
					}
				}
			};

			eventBus.addEventListener('overlay', handleNewData);
